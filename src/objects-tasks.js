...
function mergeObjects(objects) {
  ...
  objects.reduce((acc, curr) => {
    Object.entries(curr).forEach((key) => {
      if (Object.hasOwn(acc, key[0])) {
        acc[key[0]] += curr[key[0]];
      } else {
        acc[key[0]] = curr[key[0]];
      }
    });
...
function compareObjects(obj1, obj2) {
  return Object.values(obj1).every(
    (value, index) => value === Object.values(obj2)[index]
  );
}
...
function makeWord(lettersObject) {
  ...
  const arrEntries = [];
  Object.entries(lettersObject).map((arr) => {
    return arrEntries.push(arr.flat());
  });
  for (let i = 0; i < lengthWord; i += 1) {
    str += arrEntries.filter((arr) => arr.includes(i)).flat()[0];
  }
  return str;
}
...
function sellTickets(queue) {
  ...
  return queue[queue.length - 1] - 25 <= change - queue[queue.length - 1];
}
...
function fromJSON(proto, json) {
  ...
  const protObj = Object.create(proto);
  return Object.assign(protObj, obj);
}
...
function group(array, keySelector, valueSelector) {
  ...
  const setKeys = array.map(keySelector);
  setKeys.forEach((key) => {
    resMap.set(
      key,
      array
